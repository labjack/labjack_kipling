...
describe('live device scanner', function() {
	return;
	this.skip();
	...
	it('open first device', function (done) {
		capturedEvents = [];
		var params = {
			'deviceType': 'LJM_dtT7',
			'connectionType': 'LJM_ctUSB',
	...
	it('get first device attributes', function (done) {
	...
	it('get second device attributes', function (done) {
				...
				devices.forEach(function(device) {
					...
					keys.forEach(function(key) {
						if(typeof(device[key]) !== 'undefined') {
							...
							assert.isOk(false, 'Missing required key: ' + key);
						}
					});
				});
			});
