...
var module_manager;
try {
    module_manager = require('ljswitchboard-module_manager');
} catch(err) {
    // console.warn('Error!! requiring module_manager', err);
}
try {
    module_manager = global.require('ljswitchboard-module_manager');
} catch(err) {
    console.warn('Error!! global.requiring module_manager', err);
}
...
exports.renderCachedTemplateData = function(key, template, context) {
	...
	if(FS_FACADE_TEMPLATE_CACHE.has(key)) {
		...
		defered.resolve(curTemplate(context));
	} else {
		curTemplate = handlebars.compile(template);
