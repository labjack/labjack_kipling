...
function printTestStartupInfo(name) {
	var path = 'C:\\Users\\chris\\git\\LabJack\\pm\\ljswitchboard-module_manager\\test_persistent_data_dir\\device_selector\\data.json';
	var data = '';
	try {
...
var checkDataManagerStatus = function(dataManager) {
	var defered = q.defer();
	console.log('in checkDataManagerStatus', dataManager);
...
var initializeDataManager = function(dataManager) {
	var defered = q.defer();
	console.log('in initializeDataManager', dataManager);
...
var validateDataManager = function(dataManager) {
	var defered = q.defer();
	console.log('in validateDataManager', dataManager);
...
var validateManagedModuleData = function(dataManager) {
	printTestStartupInfo('validateManagedModuleData');
	console.log('dataManager info', dataManager);
	var defered = q.defer();
...
var returnModuleData = function(dataManager) {
	var defered = q.defer();
	printTestStartupInfo('returnModuleData');
...
var innerLoadModuleStartupData = function(data) {
	...
	printTestStartupInfo('innerLoadModuleStartupData');
...
var innerLoadModuleData = function(data) {
	...
	printTestStartupInfo('innerLoadModuleData');
	// Wait for all of the operations to complete
	q.allSettled(promises)
	.then(function(collectedData) {
		printTestStartupInfo('resolving innerLoadModuleData');
		defered.resolve(data);
	}, defered.reject);
	return defered.promise;
};
...
var loadModuleDataByName = function(moduleName) {
	...
	printTestStartupInfo('loadModuleDataByName');
