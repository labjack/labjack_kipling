...
var writeAndValidateDataManagerFile = function(dataManager) {
	...
	fs.writeFile(MODULES_PERSISTENT_DATA_FILE,data, function(err) {
		if(err) {
			fs.writeFile(MODULES_PERSISTENT_DATA_FILE, data, function(errB) {
				if(errB) {
					defered.reject(errB);
				} else {
					dataManager.isDataManagerInitialized = true;
...
var writeAndValidateModuleDataFile = function(dataManager) {
	...
	// Write the data to the file.
	fs.writeFile(moduleDataFilePath,data, function(err) {
		if(err) {
			fs.writeFile(moduleDataFilePath, data, function(errB) {
				if(errB) {
