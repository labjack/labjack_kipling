...
var createModuleInstance = function() {
	...
	function getHRMissingDirectConnectKey(key) {
		if(key === 'dt') {
			return 'LJM Device Type';
		} else if(key === 'ct') {
			return 'LJM Connection Type';
		} else if(key === 'id') {
			return 'LJM Identifier';
		} else {
			return 'Unknown';
		}
	}
	this.attemptDirectConnection = function(data) {
		...
		if(missingValues.length > 0) {
			var errStr = 'Please define the following missing connection parameter';
			if(missingValues.length > 1) {
				errStr += 's';
			}
			errStr += ': ';
			errStr += missingValues.join(', ');
			showInfoMessage(errStr);
		} else {
