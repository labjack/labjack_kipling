...
function luaDeviceController() {
    ...
    this.checkScriptFWCompatibility = function() {
        var dtn = self.device.savedAttributes.deviceTypeName;
        var fwVer = self.device.savedAttributes.FIRMWARE_VERSION;
        var source = self.codeEditorDoc.getValue();
        source = JSON.parse(JSON.stringify(source));
        var isError = false;
        var errorMessage = '';
        var i = 0;
        for (i = 0; i < self.functionReqs.length; i++) {
            var functionReq = self.functionReqs[i];
            var strToCheck = '';
            var reqObj = null;

            functionReq.minVersions.some(function(req) {
                if(req.dtn == dtn && fwVer < req.fw) {
                    strToCheck = functionReq.str;
                    reqObj = req;
                    return;
                }
            });
            console.log(dtn, fwVer, strToCheck, reqObj)
            if(strToCheck.length > 0) {
                isError = source.indexOf(strToCheck) >= 0;
                if(isError) {
                    var line = 0;
                    var splitSrc = source.split('\n');
                    for (var j = 0; j < splitSrc.length; j++) {
                        if(splitSrc[j].indexOf(strToCheck) >= 0) {
                            line = j + 1;
                            break;
                        }
                    }
    ...
    this.checkForCodeErrors = function() {
        ...
        if(self.isLuaCodeError()) {
            ...
            var compat = self.checkScriptFWCompatibility();
            if(compat.isError) {
                showAlert(compat.message);
                codeDeferred.reject();
            } else {
    ...
    this.copyDebuggingConsoleText = function() {
        var defered = q.defer();
        self.debuggingLog.editor.selectAll();
        var txt = self.debuggingLog.editor.getCopyText();
        self.debuggingLog.editor.clearSelection();
    ...
    this.clearDebuggingConsoleText = function() {
        var defered = q.defer();
        self.debuggingLog.editor.selectAll();
        self.debuggingLog.editor.removeLines();
        defered.resolve();
        return defered.promise;
    };
    ...
    this.setCodeEditor = function(codeEditor) {
        ...
        override(codeEditor.editor.renderer, "screenToTextCoordinates", function(old) {
            return function(x, y) {
    ...
    this.setDebuggingLog = function(debuggingLog) {
        ...
        self.debuggingLogDoc = debuggingLog;
        ...
        // Monkeypatch ace editor "zoom" functionality:
        // https://github.com/ajaxorg/ace/issues/2475#issuecomment-364266978
        override(debuggingLog.editor.renderer, "screenToTextCoordinates", function(old) {
            return function(x, y) {
                var zoom = getZoom(this.container);
                return old.call(this, x/zoom, y/zoom);
            };
