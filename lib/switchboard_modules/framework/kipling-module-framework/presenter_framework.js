...
try {
    ...
    console.log('error loading fs_facade presenter_framework', err, err.stack);
}
...
function Framework() {
    ...
    this.getDeviceErrorElements = function(serialNumber) {
        ...
        var iconSelector = getDeviceErrorJquerySelector(
            serialNumber,
            ['.status-icon']
        );
        ...
        elements.statusIcon = $(iconSelector);
    ...
    this._SetActiveDevices = function(newActiveDevices) {
        ...
        newActiveDevices.forEach(function(device) {
            ...
            console.log('Caching Device Error Elements');
            console.log('SHOULD LINK TO A DEVICE EVENT EMITTER......');
    ...
    **/
    ...
    this.qHideUserTemplate = function() {
        console.log('in qHideUserTemplate');
        var innerDeferred = q.defer();
        self.jquery.fadeOut(
            DEVICE_VIEW_TARGET,
            FADE_DURATION,
    ...
    this.qExecOnCloseDevice = function(device) {
        console.log('in qExecOnCloseDevice');
        var innerDeferred = q.defer();
    ...
    var updateSelectedDeviceList = function(data) {
        ...
        self._SetSelectedDevices([data.activeDevice]);
        console.log('Updated Selected Device', data.activeDevice.savedAttributes.serialNumber);
    ...
    this.qUpdateActiveDevice = function() {
        console.log('in qUpdateActiveDevice');
        var defered = q.defer();

        var data = {
            'deviceListing': undefined,
    ...
    **/
    ...
    this._changeSelectedDeviceUI = function (data) {
        ...
        var serialNumber;
        try {
            serialNumber = self.jquery.get(
                '.device-selection-radio:checked'
            ).val();
            // console.log('click!', data);
        } catch(err) {
            console.log('error in _changeSelectedDeviceUI', err);
        }
        ...
        .then(getSaveSelectedDevice(serialNumber), self.qExecOnLoadError)
    ...
    **/
    ...
    this.qConfigureTimer = function() {
        ...
        if((typeof(sdModule) !== 'undefined') && (typeof(sdFramework) !== 'undefined')) {
            ...
            console.log('sdModule or sdFramework not defined!!');
        }
        innerDeferred.resolve();
        return innerDeferred.promise;
    };
    ...
    var reportFinishedDaqLoop = function() {
        var defered = q.defer();
    ...
    var updatedeviceConnectivityStatus = function() {

    };
    this.runConnectedDevicesMonitor = function() {
        var defered = q.defer();
        defered.resolve();
        return defered.promise;
    };
    ...
    var configurePausedLoopTimer = function() {
        var defered = q.defer();
        setTimeout(self.runLoopIteration, self.pausedRefreshRate);
        defered.resolve();
        return defered.promise;
    };
    this.runLoopIteration = function() {
        ...
        if(self.isDAQLoopPaused) {
            ...
            var promises = [];
            promises.push(self.runConnectedDevicesMonitor());
            promises.push(self.runDAQLoop());
