...
class DataTableDataFormatter {
    constructor() {
        ...
        this.registerMatrixDataFormatter = [{
            ...
            'key': (options) => {
                const devices = [];
                const constant = options.minified;
                if (constant.devices) {
                    constant.devices.forEach((device) => {
                    ...
                    console.log('Constant is not associated with a device!', constant);
                }
                return devices.join(', ');
            },
            'isVisible': false,
     ...
     **/
    updateCachedRegisterData() {
        // Clear the cache
        this.cachedRegisterData = [];

        // Reset the registerIndex counter
        let registerIndex = 0;

        // Loop through and expand all of the constants.
        modbus_map.minifiedConstants.forEach(function(minifiedConstant) {
            ...
            const attributes = [];
            this.registerMatrixDataFormatter.forEach(function(data) {
                let attr = '';
                if (typeof(data.key) === 'function') {
                    try {
                        attr = data.key({
