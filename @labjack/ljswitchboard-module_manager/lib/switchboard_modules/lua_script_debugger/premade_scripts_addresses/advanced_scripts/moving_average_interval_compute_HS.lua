-- Implement a moving-average script by keeping track of a moving sum.
--
-- The number of values currently cached & used for averaging
-- can be read through the USER_RAM0_U32 register.
-- 
-- Note: AIN values are saved as floating-point numbers and not binary values
-- so the accuracy of the value may slowly drift.  
-- Run the  "moving_average_interval_compute_LS" example if this is required.
...
while true do
  if checkInterval(0) then --interval completed, collect AIN values
    for i=1,numChs do
      ...
      sums[i] = sums[i] + tVal - oVal -- Add tVal to total sum & subtract the value being removed.
